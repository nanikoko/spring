...
function picture_go(){
   ...
   var form = $('form[role="imageForm"]'); //제이쿼리객체
   var picture = form.find('[name=pictureFile]')[0]; //js(자바스크립트)객체
   
   //이미지 확장자 jpg 확인
   var fileFormat = picture.value.substr(picture.value.lastIndexOf(".")+1).toUpperCase();
	if(!(fileFormat=="JPG" || fileFormat=="JPEG")){
   		alert("이미지는 jpg/jpeg 형식만 가능합니다.");
   		picture.value=""; //value - 파일이름     
   		return;
	} 

	//이미지 파일 용량 체크
   if(picture.files[0].size>1024*1024*1){ //files - 파일
      alert("사진 용량은 1MB 이하만 가능합니다.");
      picture.value="";
      return;
   };
   
   //업로드 확인변수 초기화(사진변경) - 파일선택해서 이미지 변경될 경우
...
function upload_go(){
	...
	if(!$('input[name="pictureFile"]').val()){ //==null 하지 않아도 됨(자바스크립트)
		alert("사진을 선택하세요.");
