...
<!-- 화면처리(출력) - handlebars -> out.print사용 안하기 위해서 -->
...
<script>
	window.onload=function(){ //새로고침할 경우 상태유지
...
<script>
function goPage(url,mCode){ //url은  아이프레임을 	변경, mCode는 샹태유지
	//alert(url);
	$('iframe[name="ifr"]').attr("src",url);
	
	//HTML5 지원브라우저에서 사용 가능
	if (typeof(history.pushState) == 'function') { //페이지이동인 경우
	    //현재 주소를 가져온다.
	    ...
	    history.pushState(mCode, null, renewURL); //URL에 기록을 남겨줌
	} else { //페이지이동이 아닌 경우
	    location.hash = "#"+mCode; 
	}
}
...
//template->compile->action->subMenu_go()
